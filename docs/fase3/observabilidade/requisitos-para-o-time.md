...
| L9 | **`X-Correlation-Id`**: aceitar o header do cliente, repassá-lo à Lambda pelo Gateway, registrá-lo no log (campo `correlation.id`) e devolvê-lo na resposta. Se vier ausente, gerar um UUIDv4 e devolver o gerado | É o único laço entre o fluxo de autenticação e o de aplicação: os dois saem do Gateway de forma independente, cada um com o seu trace, e o `traceparent` não os cruza. Decidido pelo grupo em 2026-08-23 — ver `README.md` §3 e ADR-006. Sem devolver o header, o cliente não tem como reusar o mesmo valor na segunda chamada |
